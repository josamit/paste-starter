...
const getSkewOffset = (width: number, skewAngle: number): number =>
  Math.floor((Math.tan(skewAngle) * width) / 2);

// -9deg in radians.
export const HOMEPAGE_SKEW_ANGLE = -0.15708;

// Make this shareable between several components implementing skew (footer, hero)
export function useSlantedSkew(
  angle: number | undefined = HOMEPAGE_SKEW_ANGLE
): [number] {
...
export const SlantedBackgroundGradient: React.VFC<BackgroundGradientProps> = ({
  startColor,
  endColor,
