...
const StyledModalDialogOverlay = animated(
  styled(ModalDialogPrimitiveOverlay)(
    ...
    // import Paste Theme Based Styles due to
    // portal positioning. reach portal is a
    // sibling to the main app, so you are
    // now no longer a child of the theme
    // provider. We need to re-set some of
    // the base styles that we rely on
    // inheriting from such as font-family
    // and line-height so that compositions
    // of paste components in the modal are
    // styled correctly
    pasteBaseStyles
  )
);
...
const MobileNavigation: React.VFC<MobileNavigationProps> = ({
  ...
  const transitions = useTransition(isOpen, {
    ...
    enter: {
      width: ModalDimensions.MAX_WIDTH,
      height: ModalDimensions.MAX_HEIGHT,
      opacity: 1,
    },
            ...
            >
              ...
              >
                <Box position="fixed" top="24px" right="24px" zIndex="zIndex10">
                  <Button
                    data-testid="close_main_navigation"
